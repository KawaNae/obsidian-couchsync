...
export function tabMaintenance(this: ObsidianLiveSyncSettingTab, el: HTMLElement): void {
    // --- Lock warnings (at top) ---
    ...
    // --- Troubleshooting ---
    el.createEl("h3", { text: $msg("Setting.TroubleShooting"), cls: "setting-section-header" });
    ...
    new Setting(el).setName($msg("Prepare the 'report' to create an issue")).addButton((button) =>
        button
...
${stringifyYaml({
    ...
    // --- Recovery & Repair ---
    el.createEl("h3", { text: "Recovery & Repair", cls: "setting-section-header" });
    ...
    const resultArea = el.createDiv({ text: "" });
    new Setting(el)
    ...
    // --- Sync Operations ---
    el.createEl("h3", { text: "Sync Operations", cls: "setting-section-header" });
    ...
    // --- Reset ---
    el.createEl("h3", { text: "Reset", cls: "setting-section-header" });

    new Setting(el).setName("Back to non-configured").addButton((button) =>
        button
            .setButtonText("Back")
            .setDisabled(false)
            .onClick(async () => {
    ...
    new Setting(el).setName("Delete all customization sync data").addButton((button) =>
        button
